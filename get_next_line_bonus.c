...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/09 00:51:05 by aneekhra          #+#    #+#             */
/*   Updated: 2023/12/09 00:58:10 by aneekhra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*print_new_line(char *base_buffer, int i)
{
	char	*str;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > 256)
		return (0);
	if (!base_buffer[fd])
		base_buffer[fd] = ft_strdup("");
	base_buffer[fd] = ft_read(fd, base_buffer[fd]);
	if (!base_buffer[fd])
		return (NULL);
	line = print_new_line(base_buffer[fd], i);
	base_buffer[fd] = ft_save_string(base_buffer[fd]);
	return (line);
}
